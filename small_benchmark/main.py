...
def train_aux(model, optimizer, grad_norm=None):
    ...
    # if model.history_series_nums[0].length <=2:
    #     return

    # for i in range(len(model.history_series_nums)):
    #     for j in range(model.history_series_nums[i].length-1):
    #         prediction = model.corrected_history_series_nums[i].pull(-2)
    #         truth = model.history_series_nums[i].pull(-1)
    #         loss = criterion(truth.detach(), prediction)
    #         loss.backward(retain_graph=True)
...
def main(conf):
    ...
    for run in range(params.runs):
        ...
        for epoch in range(params.epochs):
            ...
            """
            with torch.no_grad():
                for j in range(model.corrected_history_series_nums[0].length-1):
                    if j % 4 == 0:
                        loss_aux = 0.
                        loss_no = 0.
                    for i in range(len(model.corrected_history_series_nums)):
                        data_aux = model.corrected_history_series_nums[i].pull(j)
                        data_no = model.history_series_nums[i].pull(j)
                        truth = model.history_series_nums[i].pull(j+1)
                        loss_aux += criterion_auxiliary(truth.cpu(), data_auxc.detach().cpu()).detach().numpy()
                        loss_no += criterion_auxiliary(truth.cpu(), data_no.detach().cpu()).detach().numpy()
                    if (j+1) % 4 == 0:
                        loss_aux_list.append(loss_aux/4)
                        loss_no_list.append(loss_no/4)
            """
        ...
        np.save(mkdir_path + '/' + str(run) + '_' + 'test_acc_list' + '.npy', test_acc_list)
        np.save(mkdir_path + '/' + str(run) + '_' + 'loss_precorrect_list' + '.npy', loss_no_list)
        np.save(mkdir_path + '/' + str(run) + '_' + 'loss_aftercorrect_list' + '.npy', loss_aux_list)
        """
        for i in range(0, len(model.auxiliary_models)):
            path = '/workspace/home/ubuntu/digest_yzy/pyg_autoscale/small_benchmark/auxiliary_models_trained_1/'  + 'run_' + str(run) +  '_layer_' + str(i) + '.pt'
            torch.save(model.auxiliary_models[i], path)
        """

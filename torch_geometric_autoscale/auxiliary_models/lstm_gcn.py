...
class lstm_gcn(torch.nn.Module):
    ...
    def forward(self, input_aux, edges):
        ...
        #print("input_aux.device", input_aux.device)
        #print("h0.device", h0.device)
        #print("c0.device", c0.device)
        #print("next(self.lstm.parameters()).device", next(self.lstm.parameters()).device)
        #print("next(self.lstm.parameters()).device", next(self.lstm.parameters()).device)
        #print("next(self.gcn.parameters()).device", next(self.gcn.parameters()).device)

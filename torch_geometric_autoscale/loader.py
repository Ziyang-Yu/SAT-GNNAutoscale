...
class SubgraphLoader(DataLoader):
    ...
    def compute_subgraph(self, batches: List[Tuple[int, Tensor]]) -> SubData:
        ...
        row = self.data.adj_t.storage.row()
        col = self.data.adj_t.storage.col()
        edge_index = torch.stack([row, col])
        edge_index = add_self_loops(edge_index)[0]
        data = Data(edge_index=edge_index, num_nodes=self.data.num_nodes)

        full_graph_nx = to_networkx(data)
        #full_graph_igraph = Graph.from_networkx(full_graph_nx)
        #sub_graph_igraph = full_graph_igraph.induced_subgraph(list(n_id.numpy()))
        #sub_graph_nx = sub_graph_igraph.to_networkx() 
        sub_graph_nx = full_graph_nx.subgraph(list(n_id.numpy()))
        src = list(np.arange(0, len(n_id)+1, 1))
        tgt = list(n_id.numpy())
        relabel_dict = {key:value for key, value in zip(tgt, src)}
        ...
        return SubData(data, batch_size, n_id, offset, count, sub_graph_adj_t)

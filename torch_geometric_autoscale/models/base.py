...
from torch_geometric_autoscale.history_series import History_Series
...
class ScalableGNN(torch.nn.Module):
    ...
    def __init__(self, num_nodes: int, hidden_channels: int, num_layers: int,
                 ...
                 buffer_size: Optional[int] = None, device=None, num_layers_gcn=3):
        super().__init__()
        ...
        for i in range(0, num_layers-1):
            self.auxiliary_models.append(MLP([hidden_channels,320,320, hidden_channels]).to(device))
        """

        self.history_series_nums = []
        for _ in range(self.num_layers-1):
            self.history_series_nums.append(History_Series(graph_size=num_nodes, emd_dim=hidden_channels))

        self.corrected_history_series_nums = []
        for _ in range(self.num_layers-1):
            self.corrected_history_series_nums.append(History_Series(graph_size=num_nodes, emd_dim=hidden_channels))
        ...
        for i in range(0, len(self.auxiliary_models)):
            path = '/workspace/home/ubuntu/digest_yzy/pyg_autoscale/small_benchmark/auxiliary_models_trained_1/'  + 'run_' + str(0) +  '_layer_' + str(i) + '.pt'
            self.auxiliary_models[i] = torch.load(path)
        """
        ...
        #self.hist_precorrect_list = []
        #self.hist_corrected_list = []
    ...
    def __call__(
        ...
        subgraph_adj: Optional[SparseTensor] = None,
        ...
        **kwargs,
        ...
        out = self.forward(x, adj_t, subgraph_adj, batch_size, n_id, offset, count, **kwargs)
    ...
    def push_and_pull(self, history, x: Tensor, layer_num, subgraph_adj,
                      batch_size: Optional[int] = None,
                      n_id: Optional[Tensor] = None,
                      #subgraph_adj: Optional[SparseTensor] = None,
                      offset: Optional[Tensor] = None,
        ...
        if not self._async:
            ...
            hist_precorrect = out.clone()
            #for param in self.auxiliary_models[layer_num].parameters():
            #    param.requires_grad = True # or True
            out.requires_grad = True
            out = self.auxiliary_models[layer_num](out)
            hist_corrected = out.clone()
            self.pool.async_push(x[:batch_size], offset, count, history.emb)
            out = torch.cat([x[:batch_size], out.detach()], dim=0)
            self.pool.free_pull()

            self.hist_precorrect_list.append(hist_precorrect.cpu())
            self.hist_corrected_list.append(hist_corrected.cpu())
            return out
